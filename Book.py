...
class Book:
    ...
    def get_pages(self, is_thumbnail: bool):
            ...
            if w < h:
                images.append(
                    Image.open(os.path.join(self.path, f)
                               ).resize(single_page_size)
                )
            else:
                images.append(
                    Image.open(os.path.join(self.path, f)
                               ).resize(double_page_size)
                )
        ...
        for i in images:
            ...
            if w < h:
                pages.append(
                    (customtkinter.CTkImage(dark_image=i, size=single_page_size))
                )
            else:
                pages.append(
                    (customtkinter.CTkImage(dark_image=i, size=double_page_size))
                )
    ...
    def get_full_cover(self):
        ...
        for f in os.listdir(self.path):
            ...
            if ext.lower() not in VALID_IMAGES:
                continue
            cover_im = Image.open(os.path.join(self.path, f))
            break
