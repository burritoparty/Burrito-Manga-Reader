...
from Functions import check_exists, indent_string, resource
...
class BookFrame(customtkinter.CTkScrollableFrame):
    ...
    def next_page(self):
        if self.current_page_num < len(self.page_list) - 1:
            ...
            self.page = customtkinter.CTkLabel(
                self.reader_window, image=self.page_list[self.current_page_num], text=""
            )
            self.page.grid(row=0, column=0, columnspan=3)

            # update the user's page counter
            self.pagenum_text = str(
                str(self.current_page_num + 1) +
                " / " + str(len(self.page_list))
            )
    ...
    def open_reader(self, book: Book):
        if self.reader_window is None or not self.reader_window.winfo_exists():
            ...
            next = Image.open(
                resource(os.path.join("button_icons", "next_icon.png")))
            ctk_next = customtkinter.CTkImage(dark_image=next)
            prev = Image.open(
                resource(os.path.join("button_icons", "prev_icon.png")))
            ctk_prev = customtkinter.CTkImage(dark_image=prev)
            exit = Image.open(resource(os.path.join(
                "button_icons", "remove_icon.png")))
            ctk_exit = customtkinter.CTkImage(dark_image=exit)

            # buttons
            next_page = customtkinter.CTkButton(
                self.reader_window,
            ...
            prev_page = customtkinter.CTkButton(
                self.reader_window,
    ...
    def name_update(self, book: Book, tag_json: str, authors_json: str):
        ...
        if (
            book.get_name() != self.book_name_entry.get()
            and self.book_name_entry.get() != ""
        ):
    ...
    def author_update(
        self, book: Book, tag_json: str, authors_json: str, authors: list[str]
    ):
    ...
    def append_new_tags(
        self, book: Book, tag_json: str, authors_json, checked_tag: str
    ):
    ...
    def open_book_description(self, book: Book, tag_json: str, authors_json: str):
        if self.book_window is None or not self.book_window.winfo_exists():
            ...
            self.book_window.geometry("%d+%d" % (340, 220))
            ...
            self.read_button = customtkinter.CTkButton(
                self.book_window,
                compound="top",
                ...
                command=lambda: self.open_reader(book),
                text=indent_string(book.get_name()),
            )

            # labels
            book_link_label = customtkinter.CTkLabel(
                self.book_window,
                text="Link",
                font=("Roboto", 20),
                text_color=light_pink,
            )
            book_name_label = customtkinter.CTkLabel(
                self.book_window,
                text="Name",
                font=("Roboto", 20),
                text_color=light_pink,
            )
            book_author_label = customtkinter.CTkLabel(
                self.book_window,
                text="Author",
                font=("Roboto", 20),
                text_color=light_pink,
            ...
            update = Image.open(
                resource(os.path.join("button_icons", "update_icon.png"))
            )
            ctk_update = customtkinter.CTkImage(dark_image=update)

            # buttons
            self.book_link_button = customtkinter.CTkButton(
                self.book_window,
                text="Update Link",
                image=ctk_update,
                fg_color=light_pink,
                hover_color=dark_pink,
                text_color=black,
                command=lambda: self.link_update(book, tag_json, authors_json),
            )
            self.book_name_button = customtkinter.CTkButton(
                self.book_window,
                text="Update Name",
                image=ctk_update,
                fg_color=light_pink,
                hover_color=dark_pink,
                text_color=black,
                command=lambda: self.name_update(book, tag_json, authors_json),
            )
            book_author_button = customtkinter.CTkButton(
                self.book_window,
                text="Update Author",
                image=ctk_update,
                fg_color=light_pink,
                hover_color=dark_pink,
                text_color=black,
                command=lambda: self.author_update(
                    book, tag_json, authors_json, authors
                ),
            )
            ...
            for i in tags:
                check_box = customtkinter.CTkCheckBox(
                    ...
                    fg_color=dark_pink,
                    text_color=light_pink,
                    command=lambda i=i: self.append_new_tags(
                        book, tag_json, authors_json, i
                    ),
                )
        ...
        if self.book_count == 0:
            ...
            while counter < self.books_per_page:
                button = customtkinter.CTkButton(
                    ...
                    command=lambda x=books[counter]: self.open_book_description(
                        x, tag_json, authors_json
                    ),
                    fg_color="transparent",
                    hover_color=dark_pink,
                    text=indent_string(books[counter].get_name()),
                    font=("Roboto", 18),
                )
    ...
    def initialize_self(self):
        ...
        self.excess_books = self.books_per_page - (
            math.ceil(self.book_count / self.books_per_page) *
            self.books_per_page
            - self.book_count
        )
        # start_time = time.time()
    ...
    def load_tab_tag(self, tag_cbox, tag_json, authors_json):
        ...
        for i in books_json["book"]:
            ...
            if tag_cbox.get() in tags:
                ...
                book = Book(i["path"], i["name"], i["author"],
                            i["link"], i["tagged"])
                # make the button
                button = customtkinter.CTkButton(
                    self,
                    compound="top",
                    image=book.get_cover(),
                    command=lambda x=book: self.open_book_description(
                        x, tag_json, authors_json
                    ),
                    text=indent_string(book.get_name()),
                    fg_color="transparent",
    ...
    def sort_by_tag_call(self, tag_json: str, authors_json: str):
        if (
            self.sort_by_tag_window is None
            or not self.sort_by_tag_window.winfo_exists()
        ):
            ...
            tag_cbox = customtkinter.CTkComboBox(
                self.sort_by_tag_window, values=tags, width=300
            )
            ...
            # button to submit
            tag_button = customtkinter.CTkButton(
                self.sort_by_tag_window,
                text="Filter",
                fg_color=light_pink,
                text_color=black,
                hover_color=dark_pink,
                command=lambda: self.load_tab_tag(
                    tag_cbox, tag_json, authors_json),
            )
    ...
    def sort_by_author_call(self, tag_json: str, authors_json: str):
        if (
            self.sort_by_author_window is None
            or not self.sort_by_author_window.winfo_exists()
        ):
            ...
            author_cbox = customtkinter.CTkComboBox(
                self.sort_by_author_window, values=authors, width=300
            )
            ...
            # button to submit
            tag_button = customtkinter.CTkButton(
                self.sort_by_author_window,
                text="Filter",
                fg_color=light_pink,
                text_color=black,
                hover_color=dark_pink,
                command=lambda: self.load_tab_author(
                    author_cbox, tag_json, authors_json
                ),
            )
            tag_button.grid(padx=10, pady=10)
    ...
    def search_by_name_call(self, tag_json: str, authors_json: str):
        if (
            self.search_by_name_dialogue is None
            or not self.search_by_name_dialogue.winfo_exists()
        ):
            ...
            if search_for != "" and search_for is not None:
                ...
                for i in books_json["book"]:
                    ...
                    if search_for.lower() in i["name"].lower():
                        ...
                        book = Book(
                            i["path"], i["name"], i["author"], i["link"], i["tagged"]
                        )
                        # make the button
                        button = customtkinter.CTkButton(
                            self,
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.sort_by_tag_window: customtkinter.CTkToplevel | None = None
        self.sort_by_author_window: customtkinter.CTkToplevel | None = None
        self.search_by_name_dialogue: customtkinter.CTkInputDialog | None = None

        button_frame = customtkinter.CTkFrame(self)

        filter = Image.open(
            resource(os.path.join("button_icons", "filter_icon.png")))
        ctk_filter = customtkinter.CTkImage(dark_image=filter)

        search = Image.open(
            resource(os.path.join("button_icons", "search_icon.png")))
        ctk_search = customtkinter.CTkImage(dark_image=search)

        sort_by_tag_button = customtkinter.CTkButton(
            button_frame,
            text="Filter by tag",
            width=520,
            image=ctk_filter,
            ...
            text_color=black,
            hover_color=dark_pink,
            command=lambda: self.sort_by_tag_call(tag_json, authors_json),
        )
        ...
        search_button = customtkinter.CTkButton(
            ...
            text_color=black,
            hover_color=dark_pink,
            command=lambda: self.search_by_name_call(tag_json, authors_json),
        )

        sort_by_author_button = customtkinter.CTkButton(
            button_frame,
            ...
            text_color=black,
            hover_color=dark_pink,
            command=lambda: self.sort_by_author_call(tag_json, authors_json),
        )
...
class TabNavigator(customtkinter.CTkFrame):
    def nav_first_tab(self, book_frame: BookFrame, tag_json, authors_json):
        ...
        if book_frame.current_tab + 1 < 10:
            self.tab_tracker.configure(
                text=(
                    "0"
                    + str(book_frame.current_tab + 1)
                    + " / "
                    + str(book_frame.get_tab_count())
                )
            )
        else:
            self.tab_tracker.configure(
    ...
    def __init__(
        ...
        **kwargs,
        ...
        jump_first = Image.open(
            resource(os.path.join("button_icons", "jump_first_icon.png"))
        )
        ctk_jump_first = customtkinter.CTkImage(dark_image=jump_first)

        jump_last = Image.open(
            resource(os.path.join("button_icons", "jump_last_icon.png"))
        )
        ctk_jump_last = customtkinter.CTkImage(dark_image=jump_last)

        self.tab_tracker = customtkinter.CTkLabel(
            self, text=("01 / " + str(book_frame.get_tab_count())), font=("Roboto", 20)
        )

        first_tab = customtkinter.CTkButton(
            self,
            text="First Tab",
            image=ctk_jump_first,
            command=lambda: self.nav_first_tab(
        ...
        last_tab = customtkinter.CTkButton(
            self,
            text="Last Tab",
            compound="right",
            image=ctk_jump_last,
            command=lambda: self.nav_last_tab(
                book_frame, tag_json, authors_json),
            fg_color=light_pink,
            text_color=black,
            hover_color=dark_pink,
        ...
        next_tab = customtkinter.CTkButton(
            self,
            text="Next Tab",
            image=ctk_next,
            compound="right",
            command=lambda: self.nav_next_tab(
                book_frame, tag_json, authors_json),
            width=520,
            fg_color=light_pink,
            text_color=black,
        ...
        prev_tab = customtkinter.CTkButton(
            self,
            text="Previous Tab",
            image=ctk_prev,
            compound="left",
            command=lambda: self.nav_prev_tab(
                book_frame, tag_json, authors_json),
            width=520,
            fg_color=light_pink,
            text_color=black,
